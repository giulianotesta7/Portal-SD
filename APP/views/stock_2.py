...
@router.get("/stock_2", response_class=HTMLResponse)
@restrict_users(["user_RRHH"])
async def get_stock_2(request: Request, db: Session = Depends(get_db)):
    dispositivos = db.query(dispositivo).filter(dispositivo.organismo == "stock_2").order_by(dispositivo.nombre.asc()).all()
    consumibles = db.query(consumible).filter(consumible.organismo == "stock_2").order_by(consumible.nombre.asc()).all()
    user = db.query(User).filter(User.id == request.session["user_id"]).first()
    return templates.TemplateResponse("stock_2.html", {
        "request": request,
...
@router.post("/stock_2/update_consumible")
@restrict_users(["user_RRHH"])
async def update_consumible(
    ...
    if consumible_item:
        ...
        db.commit()
    return RedirectResponse(url="/stock_2", status_code=303)


@router.post("/stock_2/update_dispositivo")
@restrict_users(["user_RRHH"])
